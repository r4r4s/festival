...
description: Generates Angular code and provides architectural guidance. Trigger when creating projects, components, or services, or for best practices on reactivity (signals, linkedSignal, resource), forms, dependency injection, routing, SSR, accessibility (ARIA), animations, component styling, testing, or CLI tooling.
license: MIT
metadata:
...
3. Once you finish generating code, run the project gate before committing:
   ```bash
   npm run lint && npm test -- --run
   ```
   Both commands must exit `0`. If either fails, fix the underlying cause before proceeding.
...
> **Note**: Tailwind CSS is explicitly out of scope for this project. Use SCSS with design tokens (`--fv-*` namespace) per the `theming-styling` skill.
...
- **End-to-End (E2E) Testing**: Best practices for E2E tests with Playwright. Read [e2e-testing.md](references/e2e-testing.md)
