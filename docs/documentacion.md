...
├── festival-list/       → Página de catálogo / listado de festivales, cargada vía `/festivales`.
│   ├── feature/
│   │   ├── festival-list.page.ts    → Página smart standalone (OnPush). Signals para búsqueda,
│   │   │                              filtro de categoría y modo de vista (tarjetas/lista).
│   │   │                              Computed `filteredFestivals` filtra catálogo por categoría y query.
│   │   ├── festival-list.page.html  → Header con título i18n + subtítulo + barra de búsqueda +
│   │   │                              botón filtro. Pills de género con dot de color. View switcher
│   │   │                              tarjetas/lista. Sección «Destacados» con grid de cards.
│   │   ├── festival-list.page.scss  → Layout responsive: 1 col (mobile) → 2 col (md) → 4 col (lg).
│   │   │                              Tokens semánticos `--fv-*`, glass pills, focus-ring.
│   │   └── festival-list.page.spec.ts → Test de creación del componente.
│   ├── ui/
│   │   └── festival-card/           → Card presentacional de festival con poster, badge, meta.
│   │       ├── festival-card.ts        → Componente (OnPush). Inputs: festival. Outputs: navigate,
│   │       │                             toggleFavourite. Categoría → CSS custom property para dot.
│   │       ├── festival-card.html      → Poster con NgOptimizedImage fill, date badge glass,
│   │       │                             favourite heart, category dot + label, nombre, ubicación, fechas.
│   │       └── festival-card.scss      → Hover lift + scale poster + glow. Tokens `--fv-*`,
│   │                                     glass mixin, focus-ring. Responsive aspect-ratio.
│   ├── data-access/
│   │   └── festival-catalogue.ts    → Catálogo estático de 4 festivales destacados con slugs,
│   │                                  claves i18n, categorías y rutas a pósters reales del proyecto.
...
│   ├── festival-search-catalogue.ts → Array `SEARCH_FESTIVALS` con los 6 festivales semilla para
│   │                                   la búsqueda del header: slug, name, city, province, logoSrc.
│   │                                   Reside en `@shared/data-access/` para que `layout/` pueda
│   │                                   importarlo sin violar las fronteras de arquitectura.
...
| ---------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-20 | Página `/festivales` — catálogo de festivales | Creada `features/festival-list/` con page smart (OnPush, Signals), card presentacional con poster `NgOptimizedImage fill`, badge glass, favourite heart y hover lift. Filtros por categoría (pills con dot de color), búsqueda, view switcher tarjetas/lista. Grid responsive 1→2→4 columnas. Catálogo estático de 4 festivales con pósters reales del proyecto. Ruta `/festivales` lazy-loaded en `app.routes.ts`. Claves i18n `festivals.*` propagadas a es/ca/en. Tokens semánticos `--fv-*` del design system existente. |
...
| 2026-06-26 | Buscador de festivales en el header | Añadido `shared/data-access/festival-search-catalogue.ts` (`SEARCH_FESTIVALS`, 6 entradas: slug, name, city, province, logoSrc). Actualizado `layout/nav-bar/`: botón lupa ahora abre/cierra un panel de búsqueda (signals `searchOpen`, `searchQuery`, computed `searchResults`); input con placeholder «Búsqueda Festival» (clave `nav.search.placeholder` en los tres locales); resultados en tarjetas con logo, nombre y ciudad que navegan a `/festivales/:slug`. SCSS: overlay absoluto bajo el header con backdrop para cerrar al pulsar fuera, glassmorphism ligero coherente con el diseño. Iconos: `LucideX` (cerrar) y `LucideChevronRight` (flecha resultado) añadidos al componente. |
| 2026-06-26 | Home hero CTAs enlazan a rutas reales | Los botones del hero en `home.page.html` pasan a `<a routerLink>` hacia `/festivales` y `/calendario`; `home.page.ts` importa `RouterLink`; `home.page.spec.ts` verifica los `href` de los CTAs. |
