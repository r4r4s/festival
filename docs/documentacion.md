...
| 2026-06-13 | Page transition UX (scroll top + fade-in + progress bar) | Añadidos `src/app/core/platform/page-transition.service.ts` (+ spec): gestiona el estado `hidden/loading/completing` de la barra de progreso activándola solo si la navegación supera 200 ms, con auto-limpieza en `NavigationEnd/Cancel/Error`. Nuevo componente `src/app/layout/nav-progress-bar/` (ts, html, scss, spec): barra de 3 px fija (z-index 200) con gradiente de marca y glow, animación en dos fases CSS (`fv-progress-load` → `fv-progress-complete`). `provideRouter` actualizado con `withRouterConfig({ scrollPositionRestoration: 'top' })` para restaurar el scroll al cambiar de ruta. `app.ts` escucha `NavigationEnd` y aplica la clase `.app__main--entering` (con reflow forzado para reiniciar la animación) que ejecuta `fv-page-enter` (opacity 0→1 + translateY 12px→0, 240ms `fv-ease-emphasized`). Keyframes `fv-page-enter`, `fv-progress-load` y `fv-progress-complete` añadidos a `_animations.scss`. `app.html` actualizado con `#mainEl` y `<fv-nav-progress-bar />`. |
| 2026-06-13 | Medusa Festival hero (`festival-detail`)                  | Scaffoldeada la feature `festival-detail` con el hero component (`fv-festival-hero`): split layout 42/58 (info / imagen) en `border-radius-xl`, breadcrumb, chip de género, título display Sora con letter-spacing expandido en "FESTIVAL", metadata (ubicación, fecha, rating, guardar), y dos CTAs (primario sólido + ghost). Imagen `hero-medusa-festival-2026.jpg` añadida a `src/assets/images/festivals/medusa/`. Spec actualizado con `provideRouter([])`. |
